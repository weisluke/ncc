...
\param p0 -- point to examine
\param hl -- half length of the region
...
*****************************************************************************/
template <typename T>
__device__ T get_line_x_position(Complex<T> p0, Complex<T> p1, T y)
{
	if (p0.re == p1.re)
	{
		return p0.re;
...
\param npixels -- number of pixels of the square region

\return position of corrected point
****************************************************************************/
template <typename T>
__device__ Complex<T> corrected_point(Complex<T> p0, Complex<T> p1, T hl, int npixels)
{
	T x = p0.re;
	T y = p0.im;
	if (x <= -hl)
...
/******************************************************
calculate the pixel mapping of a position


\param p0 -- x or y point position
\param hl -- half length of the square region
\param npixels -- number of pixels of the square region
...
__global__ void find_num_caustic_crossings_kernel(Complex<T>* caustics, int nrows, int ncols, double hl, int* num, int npixels)
{
	...
	for (int i = x_index; i < nrows; i += x_stride)
	{
		...
		for (int j = y_index; j < ncols - 1; j += y_stride)
		{
			...
			double xstart = point_to_pixel(pt0.re, hl, npixels);
			double ystart = point_to_pixel(pt0.im, hl, npixels);
			double xend = point_to_pixel(pt1.re, hl, npixels);
			double yend = point_to_pixel(pt1.im, hl, npixels);

			/*make complex pixel start and end positions*/
			Complex<T> pixpt0 = Complex<T>(xstart, ystart);
			Complex<T> pixpt1 = Complex<T>(xend, yend);
